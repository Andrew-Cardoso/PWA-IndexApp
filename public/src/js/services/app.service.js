...
export default class AppService {
    ...
    constructor ( db ) {
        ...
        this.dom = new DomService( () => {
            ...
            booksContainer.append( ...this.books.map( book => this._createBookNodeShortcut( book ) ) );
    ...
    async initAppAsync () {
        ...
        this.dom.booksContainer.append( ...this.books.map( book => this._createBookNodeShortcut( book ) ) );
        this.dom.rewardsContainer.append( ...this.info.rewards.map( reward => this.dom.createRewardNode( reward ) ) );

        this.dom.initEvents( e => this.searchBooks( e ), () => this.upsertBookAsync() );
    }
    ...
    searchBooks ( { target: { value } } ) {
        ...
        this.searchCountdown = setTimeout( () => {
            ...
            const booksContainer = this.dom.booksContainer;
            booksContainer.innerHTML = '';
            booksContainer.append( ...books.map( book => this._createBookNodeShortcut( book ) ) );
        }, 500 );
    }
    ...
    async nextPageAsync ( book ) {
        ...
        bookNode.replaceWith( this._createBookNodeShortcut( book ) );
        ...
        const rewards = this._sortRewardsByLevel( this.getNewRewards() ).reverse();
        ...
        await Promise.allSettled( [
            ...
            this.db.experience.put( this.experience ),
            this.db.info.put( this.info )
        ] );
    }
    ...
    async upsertBookAsync () {
        ...
        const book = new Book( {
            ...
            tags: this._extractTagsFromHtml( form.tags ),
        } );

        if ( book.title.isEmpty() || this._alreadyAddedBook( book ) ) return;

        const promises = [ this.db.books.put( book ) ];

        if ( this.editingBook && this.editingBook.title !== book.title )
            promises.push( this.db.books.delete( this.editingBook.title ) );
        ...
        const bookNode = this._createBookNodeShortcut( book );
    ...
    _alreadyAddedBook ( book ) {
        const alreadyAddedBook = this.books.some( ( { title } ) => book.title === title );
        return this.editingBook?.title
            ? alreadyAddedBook && this.editingBook.title !== book.title
            : alreadyAddedBook;
    }

    _extractTagsFromHtml ( { childNodes } ) {
        const tags = [];
        for ( const tagSpan of childNodes ) tags.push( tagSpan.innerText );
        return tags;
    }

    _sortRewardsByLevel ( rewards ) {
        return rewards.sort( ( a, b ) => b.level - a.level || b.title === 'leitor' || b.title === 'finalizado' );
    }

    _createBookNodeShortcut ( book ) {
        return this.dom.createBookNode( book, () => this.nextPageAsync( book ), () => this.editingBook = book );
    }
}
